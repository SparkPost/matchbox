...
<Prop name="children" type="node">
  The ComboBox wrapper's children. Only ComboBoxMenu and ComboBoxTextField are
  allowed
</Prop>

<Prop name="style" type="object">
...
<Prop name="emptyMessage" type="string" defaultValue="'No Results'">
  String displayed when item length is 0
</Prop>

<Prop name="items" type="array" defaultValue="[]">
...
<Prop name="menuRef" type="function">
  Maps to Downshift's refKey set in getMenuProps. refKey must be set to
  "menuRef"
</Prop>
...
<Prop name="itemToString" type="function" defaultValue="(item) => identity">
...
<Prop name="readOnly" type="bool">
  Whether the input is read only. Does not disable tag removal
</Prop>

<Prop name="removeItem" type="function" defaultValue="(item) => noop">
...
<Prop name="selectedItems" type="array" defaultValue="[]">
